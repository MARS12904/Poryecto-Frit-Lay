 ...
 * 
 * En desarrollo: simula el envío de emails
 * En producción: integra con un servicio real (SendGrid, AWS SES, Resend, etc.)
 */
 ...
 * 
 * En desarrollo: simula el envío (solo hace console.log)
 * En producción: usa Resend para enviar emails reales
 */
const emailJsServiceId =
  process.env.EXPO_PUBLIC_EMAILJS_SERVICE_ID || process.env.EMAILJS_SERVICE_ID || 'service_xg32syj';
const emailJsTemplateId =
  process.env.EXPO_PUBLIC_EMAILJS_TEMPLATE_ID || process.env.EMAILJS_TEMPLATE_ID || 'template_dhx72hh';
const emailJsPublicKey =
  process.env.EXPO_PUBLIC_EMAILJS_PUBLIC_KEY || process.env.EMAILJS_PUBLIC_KEY || 'U6JTnDcliUFxpKRSG';
const emailJsPrivateKey =
  process.env.EXPO_PUBLIC_EMAILJS_PRIVATE_KEY || process.env.EMAILJS_PRIVATE_KEY || 'aajVA4khB07FJT0C1-xe0';

export async function sendEmail(options: EmailOptions): Promise<boolean> {
  try {
    // En desarrollo: simular envío para evitar spam involuntario
    /*if (__DEV__) {
      console.log('📧 [EMAIL SIMULADO] (EmailJS)');
      console.log('  Para:', options.to);
      console.log('  Asunto:', options.subject);
      console.log('  Params:', options.templateParams);
      console.log('  💡 Configura tus claves de EmailJS para enviar correos reales.');
      return true;
    }*/
